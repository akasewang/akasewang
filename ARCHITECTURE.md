...
The Skills section sits between the navigation links and charts. A zero-height row followed by one empty 4px row forms each section gap, avoiding a divider inside the gap. This gives the Skills heading the shaded row color and the first icon row the plain color, without changing the chart rows' alternating colors. Its icons are static SVGs in `assets/skill-icons/`, displayed at 28px with accessible names. Cells use explicit center/middle alignment, and icons use `align="absmiddle"` to remove inline-image baseline spacing without CSS. Each icon links to its product website or language documentation while the SVG remains hosted in this repository. Sixteen cells per row share the outer table borders, with empty cells completing the final row and no nested-table inset. The table uses 48 logical columns to align six navigation links, sixteen skill cells, and two chart cards. GitHub controls table padding and strips custom layout CSS, so column counts remain fixed and cell dimensions cannot stay exactly square at every viewport width. Icons are maintained separately from the generated charts and do not require an icon-hosting service.
